    ...
    companion object {
        ...
        private fun escapeJsonHandle(jsonText: String) {
            ...
            if (JSON.isValidArray(jsonStr)) {
                JSON.parseArray(jsonStr, JSONObject::class.java).forEach { jsonObj -> rootList.add(this.escapeJson(jsonObj.toJSONString())) }
                etfRight.text = JSON.toJSONString(rootList, JSONWriter.Feature.PrettyFormat)
            } else {
                etfRight.text = JSON.toJSONString(this.escapeJson(jsonStr), JSONWriter.Feature.PrettyFormat)
            }
        }
